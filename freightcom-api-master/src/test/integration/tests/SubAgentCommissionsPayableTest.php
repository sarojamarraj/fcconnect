...
class SubAgentCommissionsPayableTest extends CommonMacros
{
      ...
      [
       ...
       [ 'create a parent agent',
         [ 'POST', '/api/user/agent', null, [
                                             'login' => self::randomUser('parent-agent'),
                                             'email' => self::randomEmail('parent-agent'),
                                             'firstname' => 'Parlour',
                                             'lastname' => 'Trick',
                                             'password' => self::randomPassword('parent-agent'),
         ...
         [ 'parent-agent-id' => 'id',
           'parent-agent-role-id' => 'authorities.0.id' ],
         [
          [ 'parent agent id is not null', 'parent-agent-id', '@notnull' ],
          [ 'parent agent role id is not null', 'parent-agent-role-id', '@notnull' ]
          ],
         ],
       ...
       [ 'create an agent',
         [ 'POST', '/api/user/agent', null, [
                                             ...
                                             'parentSalesAgentId' => '${parent-agent-role-id}',
                                             'commissionPercent' => '20',
                                             'address' => [
                                                           'city' => 'Hamilton',
                                                           'address1' => '1 Brock St.'
       ...
       [ 'look at the invoice',
         ...
         [
          'subtotal' => '@notnull',
          'amount' => '@notnull',
          'charges' => [ 'sum of charges equals subtotal',
                         function($charges, $map) {
              return abs($map['subtotal'] - array_reduce($charges, function($total, $charge) {
                  return $total + $charge['total'];
                }, 0)) < .005;
                         } ],
          'taxes' => [ 'sum of taxes equals amount - subtotal',
                         function($taxes, $map) {
              return $map['amount'] - $map['subtotal'] == array_reduce($taxes, function($total, $tax) {
       ...
       [ 'generate commissions expect eror',
         [ 'POST', '/api/agent/${agent-role-id}/report-commission' ],
         [],
         [],
         'Cannot compute commission on sub agent'
         ],

       [ 'generate commissions',
         [ 'POST', '/api/agent/${parent-agent-role-id}/report-commission' ],
         [ 'commission-statement-id' => '0.id' ],
         [ 'commission-statement-id' => '@notnull' ]
         ],

       [ 'List commissions reports',
         [ 'GET', '/api/commissionstatement', [ 'agentid' => '${parent-agent-role-id}' ] ],
